...
for i in range(1,n):
  ...
  if num - min_num <= k:
    ...
    win_num_max = i - l + 1
```
8 通过while查找位置后记得break跳出循环
9 dp很多时候需要n+1的长度，循环写range(1,n+1)
...
| 方式 | 代码 | 描述 | 适用场景 | 修改子对象影响原对象吗？ |
| :--- | :--- | :--- | :--- | :--- |
| 直接赋值 | `b = a` | 引用传递。只是多了一个标签。 | 不需要独立副本时 | 会 (完全同步) |
| 浅拷贝 | `b = a[:]` <br> `b = list(a)` <br> `b = a.copy()` <br> `copy.copy(a)` | 第一层独立。创建了新容器，但内部元素仍是引用。 | 列表/字典只有一层，且元素是不可变对象（如数字、字符串） | 会 (如果内部元素是可变对象) |
| 深拷贝 | `import copy`<br>`b = copy.deepcopy(a)` | 完全独立。递归复制所有层级，彻底断开联系。 | 嵌套列表、复杂对象树 | 不会 (完全隔离) |
