...
def parse_list_string(s):
    ...
    while i < len(s):
        ...
        if char == "[":
            ...
            # 如果有未处理的数字，先处理（防止类似 [1] 这种没有逗号的情况）
            if num_str:
        ...
        elif char.isdigit() or (char == "-" and (i == 0 or s[i - 1] in "[,")):
            # 如果是数字或负号（且负号前是[或,），累加到数字字符串
            num_str += char
    ...
    # 循环结束后，检查是否还有未处理的数字（针对最外层列表以数字结尾的情况，虽然通常会被']'触发）
    # 但在我们的逻辑中，']'和','都会触发数字处理，所以这里主要做最终校验
...
# --- 测试 ---
input_str = "[[0,1,0],[0,2,1],[0,0,0]]"
result = parse_list_string(input_str)
print(result)
# print("原始字符串:", input_str)
# print("转换结果:", result)
# print("类型检查:", type(result), type(result[0]))
# print("访问元素 result[1][1]:", result[1][1])
...
#     print(f"输入: {case} -> 输出: {parse_list_string(case)}")
```

# yield （相当于迭代的生成器）

## 用途（低内存占用）
有时候要自定义一段序列，比如斐波那契数列，但又不想用太多内存呢储存数组
```python
def fab(max): 
    n, a, b = 0, 0, 1 
    while n < max: 
...
推荐使用，速度更快，手写也快
```python
result = any(x > 100 for x in large_list)

result = False
for x in large_list:
    if x > 100:
        result = True
        break  # 必须手动写 break 才能实现短路
