...
在 软件版本管理中，版本号由点分割的数字组成，例如 1.2.3 和 2.8。现在，你需要编写一个函数，计算两个版本号之间的可用版本号个数。这里的可用版本号指的是所有满足 version1<x<version2的版本号x的个数。若 version1 >=version2 返回0.， 输入描述 输入两个字符串 version1和 version2，均遵循以下规定 
1.版本号由数字和点组成，且至少包含一个数字, 
2.点不会作为版本号的开头或结尾，也不会连续出现。 
3.每个数字部分的数值忽略前导零(例如1.01和1.001视为相同)
4.两个字符串中间以英文逗号分割 输出描述 输出一个整数，表示可用版本号的个数。
具体规则如下: 如果 version1 >= version2 ,返回0 否则，找到第一个不同的版本号段，假设在位置i，version1的该段值为v1，version2的该段值为v2。可用版本号的个数为v2-v1-1。 如果第一个不同段之后的版本段在version2中不全为0，则返回0.
...
### 技巧
v1和v2存在长度不一样的情况，通过补0后，就好操作了

```python

def solution():
    v1,v2 = list(input().split(","))
    arr1 = list(map(int,v1.split(".")))
    arr2 = list(map(int,v2.split(".")))
    n1 = len(arr1)
    n2 = len(arr2)
...
## P00419. 华为od机试—核酸最快检测效率 / 总最快检测效率
...
### 易错点
如果直接乘以有小数的数字，会变为浮点数，记得最后print时转换为整数
...
def solution(n,m,arr):
    ...
    for i in range(1,n+1):
        dp[i][0] = dp[i-1][0] + arr[i]*0.8
    ans = 0
    for i in range(1,n+1):
        for j in range(1,m+1):
            # 如果分配0个志愿者给i位
            dp[i][j] = max(dp[i-1][j] + arr[i]*0.8,dp[i][j])
            ...
            # 如果分配2个志愿者给i位
            if j >= 2:
                dp[i][j] = max(dp[i-1][j-2] + arr[i]*1.1,dp[i][j])

            # 如果分配3个志愿者给i位
            if j >= 3:
                dp[i][j] = max(dp[i-1][j-3] + arr[i]*1.2,dp[i][j])

            # 如果分配4个志愿者给i位
            if j >= 4:
                dp[i][j] = max(dp[i-1][j-4] + arr[i]*1.3,dp[i][j])
