...
index1  = bisect_left(arr,x)   index2  = bisect_right(arr,x)

- arr无x元素
index1 = index2 ;       x < arr[index1] =arr[index2]
- arr有一个x元素
index1 = index2 - 1 ;   x <= arr[index1] < arr[index2]
- arr有多个x元素
index1 = index2 - 1 ;   x <= arr[index1] < arr[index2]
...
def bisect_left(a, x, lo=0, hi=None):
    ...
    return lo
```
功能：在已排序序列 a 中查找元素 x 应该插入的位置，并返回最左侧的插入位置。**(返回第一个大于等于x的位置)**
...
def bisect_right(a, x, lo=0, hi=None):
    ...
    return lo
```
功能：在已排序序列 a 中查找元素 x 应该插入的位置，并返回最右侧的插入位置。**(返回第一个大于x的位置)**
